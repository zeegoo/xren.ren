...
但看懂天道运转不等于能在实战中读懂别人的代码。

当你接手一个 10 万行的新项目，IDE 一打开满屏的函数跳转、类继承、模块依赖——你本能反应是什么？是从 `main` 开始逐行读？还是先 Google 一遍"如何阅读开源项目"？

真正的金丹期码农，看代码的方式根本不是逐行读。

他们打开一个新项目，几秒钟扫一遍目录结构，再花几分钟跑一遍核心流程，整套架构已经在脑子里成型了。这种能力在修仙小说里叫"开天眼"——不是看见字面意思，而是看到灵气流动、因果脉络、设计意图。
...
修仙小说里修士有不同境界的眼：凡人视只能看见眼前物件；天眼通能看穿表象，看到灵气流动；法眼通能看穿因果，看到万物的来龙去脉。

读代码也有**凡人视、天眼通、法眼通**三个层次，你当前在哪一层？
...
金丹期工程师读新项目快的原因就在这儿：他们不读代码，他们读结构。
...
修仙类比：**自顶向下**是御剑飞行俯瞰全局，**自底向上**是闭关打坐从底层炼起。前者适合快速建立全貌，后者适合钻深技术细节。
...
理论讲完了，来点实战。我们用四种功法，读懂一个 Express Web 框架的核心请求链路。
...
凡人视会开始读 `express.json()` 的源码，研究怎么解析 JSON。读三天，你还在 JSON 解析器里打转，根本没碰到业务逻辑。

天眼通知道：先看请求从哪到哪，中间经过什么，最终去了哪。
...
中间件是 Express 设计的精髓——它就是一个函数 `(req, res, next) => {...}`，能改请求、能提前响应、能让请求继续往下一个中间件走。**中间件（Middleware）**是 Express 处理请求的核心抽象。
...
修仙类比：中间件就是功法叠加层。每一层中间件都是一道关卡——验身份、记日志、压数据、查权限——你的请求像修仙者闯关，必须一道道过；中间件不调用 `next()`，就等于把请求"挡在了山门外"。
...
金丹期码农看代码，本质上就是把文字转成图。 看到代码 → 脑子里立刻浮现出图 → 用图和别人沟通 → 用图指导自己修改。这是阅读能力的天花板。
...
*本文是「码农修仙传」系列第80篇。系列导航见 [xren.ren](https://xren.ren)*