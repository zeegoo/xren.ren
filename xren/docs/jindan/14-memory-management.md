...
记住一个核心要点：栈向下生长，堆向上生长。它们中间隔着"未使用区域"，这片区域就是程序的"灵力储备池"。如果两边都长到对方了，就会撞车——这叫**栈溢出（Stack Overflow）**或**堆溢出（Heap Overflow）**，是程序崩溃的两大常见原因。
...
于是有了**内存池（Memory Pool）**——程序一次性从堆申请一大块，然后自己管理小分配，避开频繁的 malloc/free。这是矿藏里的"宗门仓库"。
