...
但金丹期真正的试炼不是翻译，是分身——一个执行流变出多个，让它们同时跑不同的活。

你的程序只有一个执行流时，世界很简单。但凡要做点正经事——服务端要扛住一万个请求，UI 不能卡死，后台要批量处理百万条数据——修仙者就必须施展分身术：一条灵识变多条，让它们同时跑不同的活。
...
**并发（Concurrency）**：一个 CPU 在多个任务之间快速切换，看起来像同时跑。你一个人练三门功法，每门练五分钟换下一门——三门都在推进，但你一次只能练一门。

**并行（Parallelism）**：多个 CPU（或多核）真的同时跑多个任务。三个人各练一门功法，三人都在练——这才是真正的并行。
...
**N:1 模型**——多个灵识（用户态协程）共享一个天道位。轻量到极致，可以创建几十万。但只要其中一个阻塞（比如做 syscall），整个天道位都被占住，所有灵识一起卡死。N:1 能跑得快，但跑不了真并行。

**M:N 模型**——天道位和灵识解耦，由调度器动态分配。Go 语言的 goroutine 是经典实现：你可以创建一百万个 goroutine，runtime 会聪明地分配到几千个 OS 线程上跑。
...
这段代码在 Go 里运行毫无压力——10 万个 goroutine 分布在几千个 OS 线程上调度。换成 Java 的 `Thread`，光创建就够系统喝一壶。这就是 M:N 模型的优势：轻量又并行。
...
分身术真正的苦难，源于分身之间要共享资源。两个灵识同时改一个全局变量、同时拿一件法器、同时抢一个修炼位，结果不可预测。
...
并发三毒的解药，主要靠锁（Lock）。但锁本身也是毒药——锁不好，bug 更隐蔽。
...
修仙类比：粒度粗等于一把钥匙管整个宗门——所有弟子进出都卡一个门，效率低；粒度细等于每个修炼室单独锁——灵活但门多了容易丢钥匙互相卡死。
...
CAS 是 CPU 硬件支持的单条指令，不会被中断。所以 `counter += 1` 可以这样无锁实现：
...
修仙类比：修炼密室不用门锁，而用法术——"如果密室无人（expected 符合），则推门进入并设置占用标记（new_value）；如果有人（不符合），则刷新密室当前状态后重试"。整个过程不阻塞任何人，只是不断地"试探-确认-进入"。
...
- [ ] 能准确说出并发 vs 并行的区别，并各举一个场景
- [ ] 能画出 **1:1 / N:1 / M:N** 三种线程模型的对比，并说出 Go 选哪种、为什么
- [ ] 能写出一个线程安全的计数器（用锁或 CAS 都行）
- [ ] 能解释死锁的四个必要条件，并能识别代码中可能死锁的写法
...
> 🎮 **测一测**：你能脱口而出死锁的四个必要条件吗？在评论区默写一遍——写得出来，就算半个金丹期了。
>
