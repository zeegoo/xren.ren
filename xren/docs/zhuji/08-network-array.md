...
TCP 是传输层最常用的协议，要用它传送数据前，得先和对方**建立连接**。建立连接的过程就是 TCP 三次握手。
...
DNS（Domain Name System）就是**传送阵地址簿**——把宗门名字翻译成灵脉坐标。
...
DNS 把域名翻译成 IP，TCP 三次握手建立了可靠通道，现在终于可以传「信」了。HTTP 就是这封信的**格式和礼仪**。
